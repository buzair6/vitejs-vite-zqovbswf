...
interface AssetFormProps {
    // CORRECTED TYPE (TS2345): The addAsset function in App.tsx expects Omit<Asset, 'id' | 'status'>
    // because App.tsx adds the 'status' property itself. This type definition now matches.
...
function AssetForm({ addAsset }: AssetFormProps) {
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => { // Type the event
    ...
    // Pass ONLY the properties expected by addAsset (name, location, type) based on the corrected type
